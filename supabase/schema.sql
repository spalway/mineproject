...
-- Runtime helpers
--
-- Two operations the app cannot express as a single PostgREST call: a bulk
-- read-modify-write on depth, and ledger totals. Doing either client-side
-- would mean one request per row, so they live here.
-- ---------------------------------------------------------------------
...
-- Claim a sector atomically. The partial unique indexes already prevent a
-- double claim, but doing the checks in one statement avoids a race between
-- "is it free" and "take it" under concurrent clicks.
